...
<script type="text/html" id="LAY-app-{{ModelClassName}}-tableBox-bar">
    ...
    <a class="layui-btn layui-btn-danger layui-btn-xs" data-dropdown="#{{ModelClassName}}TbDelDrop{{d.LAY_INDEX}}" no-shade="true">删除</a>
    <div class="dropdown-menu-nav dropdown-popconfirm dropdown-top-right layui-hide" id="{{ModelClassName}}TbDelDrop{{d.LAY_INDEX}}"
                ...
                table.render({
                    ...
                    cols: [
                        [
						    ...
						    { field: '{{field.DbColumnName}}', title: '{{field.ColumnDescription}}', width: 80, sort: false},{% elsif field.DataType == 'datetime' %}
						    { field: '{{field.DbColumnName}}', title: '{{field.ColumnDescription}}', width: 170, sort: false},{% elsif  field.DataType == 'bit' %}
...
<script type="text/html" id="switch_{{field.DbColumnName}}">
    <input type="checkbox" name="switch_{{field.DbColumnName}}" value="[[d.id]]" lay-skin="switch" lay-text="开启|关闭" lay-filter="switch_{{field.DbColumnName}}" [[ d.{{field.DbColumnName}} ? 'checked' : '' ]]>
</script>
{% endif %}{% endfor %}
