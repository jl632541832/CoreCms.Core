...
namespace CoreCms.Net.Web.Admin.Controllers
{
    ...
    public class {{ModelClassName}}Controller : ControllerBase
    {
        ...
        public async Task<JsonResult> GetPageList()
        {
            ...
            Expression<Func<{{ModelClassName}}, object>> orderEx;
            switch (orderField)
            {
				{% for field in ModelFields %}case "{{field.DbColumnName}}":
                    orderEx = p => p.{{field.DbColumnName}};
                    break;{% endfor %}
                default:
                    orderEx = p => p.id;
                    break;
            ...
            //获取数据
            var list = await _{{ModelClassName}}Services.QueryPageAsync(where, orderEx, orderBy, pageCurrent, pageSize);
            //返回数据
            jm.data = list;
            jm.code = 0;
